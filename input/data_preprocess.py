...
class DataPreprocess():
    ...
    def visualize_degree_distribution(G, output_dir, name):
        ...
        for i in range(len(deg)):
            deg[i] = len(list(G.neighbors(list(G.nodes())[i])))
    ...
    def visualize_distribution(dist, output_dir, name):
        ...
        fig = plt.figure()
        axes = fig.gca()
        axes.hist(dist, density=False, bins=30)
    ...
    def evaluateDataset(dataset1, dataset2, groundtruth, output_dir):
        ...
        for source_key in groundtruth.keys():
            source_id = source_key
            target_id = groundtruth[source_id]
            source_idx = dataset1.id2idx[str(source_id)]
            target_idx = dataset2.id2idx[str(target_id)]
        ...
        for source_node in groundtruth.keys():
            ...
            source_neighbors = dataset1.G.neighbors(str(source_node))
            target_neighbors = dataset2.G.neighbors(str(target_node))
