...
def test(model, topk, g, x, edge_types, node_mapping1, node_mapping2, test_set, anchor_links2, dist, mode='training'):
    ...
    with torch.no_grad():
        ...
        x2_2 = x2[:, dim//2: dim]
        if dist == 'inner':
            S_l = weights[0] * x1_1.dot(x2_1.T) + weights[1] * x1_1.dot(x2_2.T) + weights[2] * x1_2.dot(x2_1.T) + \
                weights[3] * x1_2.dot(x2_2.T)
        else:
            S_l = weights[0] * manhattan_distances(x1_1, x2_1) + \
        ...
        if mode == 'testing':
            for i in range(len(S_l)):
                S_l[i][anchor_links2] = 0
        idx2 = np.argsort(-S_l, axis=1)[:, :topk[-1]]
        ...
        x2_2 = x2[:, dim // 2: dim]
        if dist == 'inner':
            S_r = weights[0] * x1_1.dot(x2_1.T) + weights[1] * x1_1.dot(x2_2.T) + weights[2] * x1_2.dot(x2_1.T) + \
                weights[3] * x1_2.dot(x2_2.T)
        else:
            S_r = weights[0] * manhattan_distances(x1_1, x2_1) + \
                weights[1] * manhattan_distances(x1_1, x2_2) + \
                weights[2] * manhattan_distances(x1_2, x2_1) + \
                weights[3] * manhattan_distances(x1_2, x2_2)
            S_r = -S_r
        S_r = expit(S_r)
        idx2 = np.argsort(-S_r, axis=0)[:topk[-1], :]
