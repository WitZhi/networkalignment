...
class IONE(NetworkAlignmentModel):
    def __init__(self, source_dataset, target_dataset, gt_train, epochs=100, dim=100, seed=123, learning_rate=0.05):
        ...
        #将训练集文件中的源节点和目标节点读入到一个字典 self.gt_train 中
        self.gt_train = {}
    ...
    def _save_data_as_ione_format(self):
        ...
        #对源数据集和目标数据集的节点进行编号，将节点编号与原节点名称建立映射关系。
        ...
        #为源数据集和目标数据集添加缺失的节点，使得源数据集和目标数据集的节点数量相同。
        ...
        #将新的边列表和训练集保存为文件，文件路径分别为 src_edgelist_file、trg_edgelist_file 和 train_gt_file。
    ...
    #884-5094_2 6155-5094
    def _create_gt(self, gt, src_map, trg_map):
        new_gt = []
        for line in gt:
            if src_map[line[0]] != trg_map[line[1]] and src_map[line[0]] == trg_map[str(line[1])+"_2"]:
                line[1] = str(line[1])+"_2"
