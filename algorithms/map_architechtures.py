...
class PaleMappingMlp(PaleMapping):
    def __init__(self, embedding_dim, source_embedding, target_embedding, activate_function='sigmoid'):
        ...
        if activate_function == 'sigmoid':
            print('use sigmoid activate function')
            self.activate_function = nn.Sigmoid()
        elif activate_function == 'relu':
            print('use relu activate function')
            self.activate_function = nn.ReLU()
        else:
            print('use tanh activate function')
    ...
    def loss(self, source_indices, target_indices):
        """ for i in range(len(source_indices)):
            if source_indices[i] >= len(self.source_embedding):
                print(source_indices[i]) """
        mapping_loss = 0
        for i in range(len(self.source_embedding)):
            source_feats = self.source_embedding[i][source_indices]
            target_feats = self.target_embedding[i][target_indices]
