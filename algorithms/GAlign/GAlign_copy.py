...
from algorithms.GAlign.embedding_model_copy import G_Align as Multi_Order, StableFactor
...
class GAlign_copy(NetworkAlignmentModel):
    """
    ...
    def __init__(self, source_dataset, target_dataset, args):
        ...
        """
        super(GAlign_copy, self).__init__(source_dataset, target_dataset)
        self.source_dataset = source_dataset
        self.target_dataset = target_dataset
        #self.alphas = [args.alpha0, args.alpha1, args.alpha2,args.alpha3, args.alpha4, args.alpha5,args.alpha6, args.alpha7]
    ...
    def align(self):
        ...
        source_A_hat = torch.tensor(source_adj)
        target_A_hat = torch.tensor(target_adj) """
        """ if self.args.cuda:
        ...
        S_GAlign = self.refinement_alignment(GAlign, source_A_hat, target_A_hat, source_feats, target_feats)
        return S_GAlign
    ...
    def multi_level_embed(self, source_A_hat, target_A_hat, source_feats, target_feats):
        ...
        for epoch in range(self.args.GAlign_epochs):
            ...
            for i in range(2):
                for j in range(len(new_source_A_hats)): ##跟GCN的层数没有关系
                    structural_optimizer.zero_grad()
                    if i == 0:
                        A_hat = source_A_hat
                        augment_A_hat = new_source_A_hats[j]
                        outputs = GAlign(source_A_hat, 's')
                        """ edge_index = torch.nonzero(torch.triu(source_A_hat) == 1, as_tuple=False).t()
                        augment_edge_index = torch.nonzero(torch.triu(augment_A_hat) == 1, as_tuple=False).t() """
                        #outputs = GAlign(source_feats,edge_index)
                        if j < 3:
                            augment_outputs = GAlign(augment_A_hat, 's')
                            #augment_outputs = GAlign(source_feats,augment_edge_index)
                        else:
                            augment_outputs = GAlign(augment_A_hat, 's', new_source_feats)
                            #augment_outputs = GAlign(new_source_feats,augment_edge_index)
                    else:
                        A_hat = target_A_hat
                        augment_A_hat = new_target_A_hats[j]
                        outputs = GAlign(target_A_hat, 't')
                        """ edge_index = torch.nonzero(torch.triu(target_A_hat) == 1, as_tuple=False).t()
                        augment_edge_index = torch.nonzero(torch.triu(augment_A_hat) == 1, as_tuple=False).t()
                        outputs = GAlign(target_feats,edge_index) """
                        if j < 3:
                            augment_outputs = GAlign(augment_A_hat, 't')
                            #augment_outputs = GAlign(target_feats,augment_edge_index)
                        else:
                            augment_outputs = GAlign(augment_A_hat, 't', new_target_feats)
                            #augment_outputs = GAlign(new_target_feats,augment_edge_index)
    ...
    def refinement_alignment(self, GAlign, source_A_hat, target_A_hat,source_feats, target_feats):
        source_A_hat = source_A_hat.to_dense()
        target_A_hat = target_A_hat.to_dense()
        GAlign_S = self.refine(GAlign, source_A_hat, target_A_hat, 0.94,source_feats, target_feats)
        return GAlign_S
    ...
    def get_elements(self):
        ...
        source_A_hat, _ = self.source_dataset.get_adjacency_matrix()
        target_A_hat, _ = self.target_dataset.get_adjacency_matrix()
    ...
    def refine(self, GAlign, source_A_hat, target_A_hat, threshold,source_feats, target_feats):
        refinement_model = StableFactor(len(source_A_hat), len(target_A_hat), self.args.cuda)
        if self.args.cuda: 
            refinement_model = refinement_model.cuda()
        S_max = None
        """ edge_index = torch.nonzero(torch.triu(refinement_model(source_A_hat, 's')) == 1, as_tuple=False).t()
        edge_index_2 = torch.nonzero(torch.triu(refinement_model(target_A_hat, 't')) == 1, as_tuple=False).t() """
        source_outputs = GAlign(refinement_model(source_A_hat, 's'), 's')
        target_outputs = GAlign(refinement_model(target_A_hat, 't'), 't')
        """ source_outputs = GAlign(source_feats,edge_index)
        target_outputs = GAlign(target_feats,edge_index_2) """
