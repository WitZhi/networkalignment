...
def get_loss(source_outputs, target_outputs, test_dict=None, alphas=None, just_S=False):
    Sf = torch.zeros((len(source_outputs[0]), len(target_outputs[0]))).cuda()
    accs = ""
    for i in range(0, len(source_outputs)):
        S = torch.matmul(F.normalize(source_outputs[i]), F.normalize(target_outputs[i]).t()).cuda()
        if test_dict is not None:
            if not just_S:
                acc = get_statistics(S.detach().cpu().numpy(), test_dict)
    ...
    if test_dict is not None:
        if not just_S:
            acc = get_statistics(Sf.detach().cpu().numpy(), test_dict)
            accs += "Final acc is: {:.4f}".format(acc)
    return accs, torch.div(Sf, len(source_outputs))
...
def get_acc(source_outputs, target_outputs, test_dict = None, alphas=None, just_S = False):
    ...
    if test_dict is not None:
        if not just_S:
            ...
            accs += "Final acc is: {:.4f}".format(acc)
    return accs, Sf/len(source_outputs)
