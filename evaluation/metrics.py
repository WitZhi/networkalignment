...
def get_statistics(alignment_matrix, groundtruth, groundtruth_matrix=None, use_greedy_match=False, get_all_metric = False):
    ...
    if get_all_metric:
        MAP, Hit, AUC,Hit30  = compute_MAP_Hit_AUC(alignment_matrix, groundtruth,30)
        pred_top_30 = top_k(alignment_matrix, 30)
        top30 = compute_precision_k(pred_top_30, groundtruth)
        ...
        return acc, MAP, top5, top10,AUC,top30,Hit30
    return acc
...
def compute_MAP_Hit_AUC(alignment_matrix, gt,k):
    MAP = 0
    AUC = 0
    Hit = 0
    Hit_k=0
