...
def parse_args():
    ...
    parser.add_argument('--input_dir', default="../graph_data/douban/online",  help="Input directory")
    parser.add_argument('--out_dir', default="../graph_data/douban/online/shuffle", help="Output directory")
...
def shuffle(all_nodes, all_edges, args):
    ...
    for edge in all_edges:
        permute_edges.append([node_dict[str(edge[0])], node_dict[str(edge[1])]])

    np.random.shuffle(permute_edges) 
    print(len(permute_edges))    
...
def save(permute_edges, node_dict, node_rev_dict, input_dir, out_dir):
    ...
    with open(out_dir + "/dictionaries/id_old_to_new", 'w') as f:
...
if __name__ == '__main__':
    ...
    all_nodes, all_edges = load_edgelist_file(args.input_dir + "/graphsage/"+"G.json")
