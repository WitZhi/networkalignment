...
def construct_adjacency(G, id2idx, sparse=False, file_path=None):
    if id2idx:
        ...
        nodes_list = list(range(len(G.nodes())))
        edges_list = list(G.edges())
        edges = np.array([[edge[0], edge[1]] for edge in edges_list])
    if file_path:
...
def build_neighbors(G, id2idx):
    neighbors = [[] for _ in range(len(G.nodes()))]
    for node in G.nodes():
        if id2idx:
            neighbor = [id2idx[i] for i in G.neighbors(node)]
            neighbors[id2idx[node]] = neighbor
        else:
...
def get_edges(G, id2idx=None):
    if id2idx:
        edges1 = [(id2idx[n1], id2idx[n2]) for n1, n2 in G.edges()]
        edges2 = [(id2idx[n2], id2idx[n1]) for n1, n2 in G.edges()]
    else:
        edges1 = [(int(n1), int(n2)) for n1, n2 in G.edges()]
        edges2 = [(int(n2), int(n1)) for n1, n2 in G.edges()]
    
    edges = edges1 + edges2
    edges = np.array(edges)
    return edges

#将真实标签文件加载为一个稀疏矩阵或字典对象，并将节点 ID 转换为索引
